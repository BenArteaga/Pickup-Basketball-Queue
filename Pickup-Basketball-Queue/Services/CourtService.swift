...
class CourtService {
    ...
    //var for the current gym if CourtService is being used on the player side
    var gymID: String = ""
    ...
    //function to load all of the courts for the given gym
    func loadCourtsForGym(completion: @escaping (Bool) -> Void) {
        courts.removeAll()
        let ref = Database.database().reference().child("courtInfo").child(self.gymID)
        ref.observeSingleEvent(of: .value, with: { (snapshot) in
            guard let allCourts = snapshot.children.allObjects as? [DataSnapshot] else {
                return completion(false)
            }
